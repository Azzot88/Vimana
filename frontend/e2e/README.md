...
**Второй аккаунт — `contact@dealvault.club`** (T_TEST.12). Нужен ровно для
одного: `invite-flow` связывает приглашением **двух** людей, а свой собственный
инвайт принять нельзя — эндпоинт отвечает 400. Заводится теми же тремя шагами.
Если эта спека не гоняется, переменные можно оставить пустыми.

**Куда положить пароли:** `frontend/e2e/.env.local` — файл в `.gitignore`,
рядом лежит `.env.example` с ключами без значений.

```
E2E_USER=niktrifonov83@gmail.com
E2E_PASSWORD=…
E2E_USER2=contact@dealvault.club
E2E_PASSWORD2=…
```

**Первый аккаунт обязан оставаться обычным пользователем.** `admin-guard`
проверяет, что роль `user` никуда не проходит; повышение до арбитра или
суперюзера покрасит его в красный. Раньше это было бесплатным свойством
одноразового аккаунта, теперь — требование к общему состоянию.
