...
С T3.28 регистрация идёт кодом на почту, и тест прочитать его не может. Решение
владельца 2026-08-22: авторизованные спеки входят под **долгоживущим аккаунтом**.

```
export E2E_USER=e2e-fixed@e2e.vimana.local
export E2E_PASSWORD='…'
npm run a11y
```

Аккаунт заводится **один раз руками**: пройти обычную регистрацию по коду,
затем задать пароль в профиле, и включить обе роли (перевозчик и отправитель) —
спеки переключают `active_mode`, но не выдают себе способности.

Домен `@e2e.vimana.local` выбран не случайно: `cleanup_e2e_users` удаляет такие
аккаунты старше суток. **Постоянный аккаунт должен быть вне этого правила** —
либо на другом домене, либо задача чистки обходит его по адресу. Иначе он
исчезнет через сутки, и прогон снова упадёт непонятно почему.
