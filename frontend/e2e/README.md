...
## Учётка для авторизованных прогонов (с 2026-08-22)

С T3.28 регистрация идёт кодом на почту, и тест прочитать его не может: код
лежит bcrypt-хешем и не пишется в логи. Решение владельца 2026-08-22 —
авторизованные спеки входят под **одним долгоживущим аккаунтом**.

**Завести один раз, руками:**

1. Зарегистрироваться обычным путём на **реальный ящик** — сейчас это
   `niktrifonov83@gmail.com`. Реальный, потому что код должен кто-то прочитать.
2. **Задать пароль на самом аккаунте** — это отдельное действие, а не запись в
   файл ниже. Регистрация кодом пароля не создаёт вовсе, а `/login` отказывает
   аккаунту без пароля ровно так же, как с неверным, — тем же 401.
3. Включить обе роли — перевозчика и отправителя. Спеки переключают
   `active_mode`, но способностей себе не выдают.

**Куда положить пароль:** `frontend/e2e/.env.local` — файл в `.gitignore`,
рядом лежит `.env.example` с ключами без значений.

```
E2E_USER=niktrifonov83@gmail.com
E2E_PASSWORD=…
```

Playwright читает его сам (загрузчик в `playwright.config.ts`). Настоящие
переменные окружения имеют приоритет, так что CI может обойтись без файла.
...
**Про чистку.** `cleanup_e2e_users` метёт только `@e2e.vimana.local`, поэтому
аккаунт на gmail ей не виден и `E2E_KEEP_EMAILS` для него не нужен — настройка
остаётся страховкой на случай возврата к тестовому домену.

**Про то, что это живой аккаунт.** Спеки действуют от его имени на dev-стенде:
переключают режим, открывают экраны. Для доступности это безобидно, но если
позже появятся сценарии, создающие рейсы и сделки, они будут копиться под этим
адресом — это нормально для dev и неприемлемо для прода.
