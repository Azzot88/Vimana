...
- [ ] `app/core/step_up.py` — грант в Redis, TTL 5 минут, ключ включает `user_id` **и область операции**. Токен от одной операции не подходит к другой: иначе подтверждение отвязки устройства молча разрешало бы объявить ключ утраченным.
- [ ] **Fail-closed при недоступности Redis** — в отличие от `token_blacklist` (см. `D-REVOCATION-IS-BEST-EFFORT`). Там fail-soft оправдан: обычные запросы важнее. Здесь наоборот — «Redis лежит, поэтому пропускаем подтверждение» перед необратимым действием звучит неверно.
- [ ] Три пруфа, по одному на каждый способ входа: пароль · WebAuthn-assertion · Nostr-подпись. Механика челленджа переиспользуется из T3.12/T3.14, добавляется свой `purpose`/scope.
- [ ] **Под step-up:** `declare-lost`, отвязка passkey, смена email, добавление нового способа входа. `establish` — **нет**: он уже требует доказательства владения новым ключом, и второе подтверждение ничего не добавляет.
