...
### T3.16 — Recovery-коды: запасной вход, не запасная личность 🟨 pt.1 готов
...
**pt.2 — фронт и уведомления ⬜**

- [ ] Экран показа кодов: «сохраните, второй раз не покажем», кнопки «Скачать .txt» и «Скопировать». Расчёт на бумагу: `IBM Plex Mono`, группировка по 4, печатная раскладка.
- [ ] Страница восстановления: идентификатор + код → привязка нового способа входа.
- [ ] Баннер «осталось N кодов» при N ≤ 2 (данные уже есть в `MeOut`).
- [ ] Письмо и in-app уведомление после использования кода — «в аккаунт вошли по коду восстановления», с остатком.
- [ ] i18n ×6.
