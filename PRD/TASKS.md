...
**pt.2 — блокирующий I/O и внешние клиенты ⬜**

- [ ] `core/storage.py` — boto3-клиент создаётся на **каждый** вызов, а `get_presigned_url` зовётся в цикле по вложениям страницы чата. Кэшировать клиент.
- [ ] `upload_file` (синхронный PUT в R2) вызывается из async-эндпоинтов — `api/avatar.py`, `api/dealvault.py`, `api/verification.py`. Пока файл заливается, event loop стоит: один пользователь с большим фото тормозит всех на этом воркере. Туда же полный декод Pillow из `core/file_validation.py`. Через `run_in_threadpool`, без смены библиотек.
- [ ] `core/nostr_publish.publish_event` — релеи обходятся последовательно с таймаутом 5 с (до 20 с на событие). `asyncio.gather`.
