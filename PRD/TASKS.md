...
**Acceptance: ✅ все выполнены** — владелец ключа входит без пароля и без email; перехваченный подписанный запрос повторно не проходит; аккаунт без пароля полностью работоспособен. **53 целевых теста зелёные** (`test_nostr_auth` + `test_identity_establish` + `test_auth`, 2026-07-27). Полный прогон с fuzz — см. ниже.
