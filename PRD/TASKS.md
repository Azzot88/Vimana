...
- [ ] Email о статусах и приближении прибытия (мягкие формулировки).
- [ ] Telegram-уведомления: поле `telegram_chat_id` в профиле; привязка через бота (`/start` → сохранить chat_id); уведомления о статусах сделки.
- [ ] WhatsApp-уведомления: поле `whatsapp_number` в профиле; интеграция через провайдера (Twilio / WATI); те же события, что и в Telegram.
- [ ] В профиле: секция «Уведомления» — включить/отключить каждый канал (email / Telegram / WhatsApp).
...
- [ ] Подключить `react-i18next`; namespace `common`, `auth`, `trips`, `deals`, `profile`.
- [ ] Языки: EN (по умолчанию), UK, PL, FR, ES. Переключатель в шапке/профиле.
- [ ] Перевести все пользовательские строки (кнопки, лейблы, сообщения об ошибках, статусы).
- [ ] Выбранный язык сохраняется в `localStorage`.
- [ ] Бэкенд: сообщения об ошибках API остаются на EN (клиент переводит по коду или ключу).
