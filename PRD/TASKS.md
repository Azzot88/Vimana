...
> 3. Подтверждение email — **мягкое**: неподтверждённый email не блокирует вход, блокирует создание сделок и рейсов.
...
- [ ] **Мягкий гейт:** `DEAL_CREATE` / `TRIP_CREATE` требуют `email_verified_at IS NOT NULL`, **если email задан**. Аккаунт без email (T3.13/T3.14) гейтом не затрагивается — у него нет email в подвешенном состоянии. Логика: «заявил email — докажи, что он твой». Вход не блокируется никогда. → Decision Log TECHSTATE.
...
**E2E (T_TEST.3 не должен покраснеть):**
- [ ] Вход не гейтится → auth-спеки живут как есть. Спеки, создающие сделки/рейсы, требуют обхода: env `E2E_AUTO_VERIFY_EMAIL_DOMAIN` (по умолчанию **пусто**) — при совпадении домена email `email_verified_at` ставится сразу при регистрации. На dev = `e2e.vimana.local` (TLD `.local` не резолвится, реальную почту не задеть), на prod переменная **не задаётся**; при непустом значении — WARNING в лог на старте приложения.

**Тесты:** request-code (202 · cooldown 429 · rate-limit) · verify (успех · неверный код · истёкший · 6-я попытка · идемпотентность) · регистрация с `phone` → 422 · логин по телефону → 401 · `DEAL_CREATE` на неподтверждённом email → 403 · аккаунт без email не гейтится · SMTP замокан (в реальный ящик не ходим).

**Acceptance:** регистрация только email+пароль; код приходит письмом и подтверждается; неподтверждённый email не мешает войти, но мешает создать сделку/рейс; Playwright-сьют зелёный; backend-сьют зелёный.
...
**API после задачи:**
- [ ] `POST /api/me/identity/challenge` → одноразовый nonce (Redis, TTL 300s), как в T3.13.
- [ ] `POST /api/me/identity/establish` `{npub, signed_proof}` — проверяет подпись челленджа новым ключом, ставит `nostr_pubkey = npub`, `key_self_custody = true`, **уничтожает** `nsec_encrypted`/`nsec_nonce`, запускает таск перешифровки. Занятый npub → 409. Одноразово: повторный вызов на аккаунте с `key_self_custody = true` → 409.
- [ ] `POST /api/me/identity/declare-lost` (под step-up T3.15) + `users.key_lost_at`. Потеря ключа ≠ потеря доступа: юзер с живым passkey входит, но подписывать и читать E2E-переписку не может. Аккаунт становится **терминальным**: запрет на создание сделок и рейсов, публикация выключена, trust-рёбра замораживаются (не удаляются — история валидна), чип «ключ утрачен» в профиле. Обратного перехода нет.
- [ ] **Удаляются:** `POST /me/keypair/claim` (повышал служебный ключ), `POST /me/keypair/import` (без доказательства владения), `POST /me/keypair/export` (служебный ключ пользователю не выдаётся — он ему не нужен и не его). Соответствующие схемы и кнопки в `KeypairSection.tsx` уходят.
- [ ] `GET /me/keypair/status` остаётся, поля переосмысляются: `identity_established: bool` вместо `key_self_custody`, `npub` отдаётся только когда личность установлена.

**Схема (миграция `0029_identity`):**
- [ ] `users.nostr_pubkey` → **NOT NULL + UNIQUE** после бэкфилла. Дублей на проде нет (проверено 2026-07-26).
- [ ] `users.key_lost_at TIMESTAMPTZ NULL`.
- [ ] `trips.nostr_published_by_pubkey VARCHAR(64) NULL` — каким ключом рейс опубликован. Нужно для NIP-09: удаление обязано быть подписано **тем же** ключом, что и публикация.
- [ ] **Бэкфилл служебных ключей** тем, у кого их нет: 12 обычных юзеров + **арбитр и superuser**. У арбитра ключа нет вообще, а `api/threshold.py:40` его требует — то есть threshold 2-of-3 сейчас не собирается. Бэкфилл чинит этот дефект попутно.

**Публикация рейсов:**
- [ ] Только под **`PLATFORM_PUBLISH_NSEC`** (новый env, ENVIRONMENT §секреты + `.env.example`), пока пользователь не установил личность. Отдельный ключ от `CHAIN_ANCHOR_NSEC` по логике из T3.6: «рейс существует» и «вот наш неизменённый head» — разные утверждения, ключи не смешиваем. Авторство события — платформа; `display_name` перевозчика в `content`, авторство пользователя не подделывается. Ключ вносится в whitelist собственного relay (`tasks/nostr_whitelist.py`).
- [ ] **Публикуются не все рейсы, а отобранные фильтром «интересных»** (решение владельца 2026-07-26): редкие направления, длинные хопы и подобное; критерии фиксируются отдельно. Это снимает риск firehose с одного ключа.
- [ ] После установления личности — публикация под ключом пользователя; ранее опубликованные платформой события гасятся kind-5 **под ключом платформы** (механика есть, `core/nostr_publish.py:107-117`) и переиздаются. `nostr_published_by_pubkey` обновляется.

**Перешифровка при переходе** (Celery-таск, идемпотентный и возобновляемый):
- [ ] DealVault уже разделяет шифрование контента (session-key → `text_ciphertext`/`text_nonce`) и доставку ключа (`read_packages`, JSON). Платформа разворачивает session-ключи служебным ключом и переупаковывает их NIP-04 на новый npub. **Байты контента не меняются** → `sha256(ciphertext||nonce)` тот же → цепь T3.6 и `verify_content` целы. Это ровно envelope-схема, предписанная D-DVLT-PROTOCOL.
- [ ] `IdentityContainer` перешифровывается целиком: его `doc_hash` считается по **plaintext**-байтам (T3.9), поэтому тройное совпадение хешей `identity_ref` переживает смену шифра.
- [ ] На проде переупаковывать сегодня нечего (0 контейнеров, 0 read_packages, 0 E2E) — корректность проверяется тестами на синтетике, требований к производительности не предъявляем.

**Копирайт перехода** (точные формулировки — в T3.17, здесь фиксируется смысл, который нельзя исказить):
- [ ] Ключ **создаётся в браузере пользователя и на сервер не попадает** — не «мы отдаём вам ключ, который хранили». Это разные утверждения, и только первое правда.
- [ ] Прямо сказать, что до перехода сейф шифровался служебным ключом платформы, сейчас его содержимое перешифровывается на ключ пользователя, а служебный уничтожается.
- [ ] Термин «DID» в коде, API и UI не вводится: везде `npub` (решение владельца №3).

**Тесты:** `establish` без доказательства владения → 422 · голый npub → 422 · занятый npub → 409 · повторный `establish` → 409 · после `establish`: `nsec_encrypted IS NULL`, `key_self_custody = true` · `claim`/`import`/`export` → 404 · **после перешифровки `verify_content` и `verify_chain` зелёные** · тройной hash-match `identity_ref` сохраняется · таск идемпотентен и возобновляем · рейс до перехода публикуется платформенным ключом · kind-5 подписан тем же ключом, что и публикация · бэкфилл выдаёт ключи всем без исключения, включая арбитра · `threshold` собирается после бэкфилла · `declare-lost` требует step-up, после него создание сделки → 403 · подписи в цепи верифицируются после `declare-lost`.

**Acceptance:** служебный ключ нигде не подаётся пользователю как его личность и не публикуется наружу; переход даёт всегда новый ключ, сгенерированный вне сервера или принесённый с доказательством владения; сейфы переезжают без нарушения цепи; `claim`, `import` и `export` больше не существуют; у всех аккаунтов, включая арбитра, есть ключ и threshold собирается.

### T3.13 — Вход и регистрация по Nostr-ключу (NIP-98)

**Контекст.** Механика проверки подписи уже есть и обкатана: `core.signing.compute_event_id` + `core.keypair.verify_event_id` используются в `api/nostr.py:78` для приёма NIP-07-подписанных событий. Здесь та же схема, но событие подтверждает не публикацию рейса, а владение ключом.

- [ ] `POST /api/auth/nostr/challenge` `{pubkey_hex}` → `{challenge, expires_in}`. Nonce 32 байта hex, Redis `nostr_challenge:{pubkey}`, TTL 300s, **одноразовый** (`GETDEL`). Redis уже подключён (`core/token_blacklist.py`).
- [ ] `POST /api/auth/nostr/verify` — принимает NIP-98 event (kind 27235; теги `u` = абсолютный URL эндпоинта, `method` = POST, `challenge` = выданный nonce; `created_at` в пределах ±60s). Проверка: id пересчитывается, подпись верифицируется, challenge гасится.
- [ ] Известный `pubkey` → JWT (`create_access_token`). Неизвестный → **404** `nostr_pubkey_unknown`; фронт предлагает регистрацию. Аккаунт молча на любую валидную подпись не создаём.
- [ ] `POST /api/auth/nostr/signup` — та же проверка подписи + `display_name`; `email` опционален. Создаёт юзера: `password_hash=NULL`, `nostr_pubkey` = заявленный (он же личность), `nsec_encrypted=NULL`, `key_self_custody=True`. Это **единственный** путь принести свой ключ — на живом аккаунте ключ не подменяется (T3.12).
- [ ] Rate-limit: `10/minute` на challenge и на verify (slowapi + nginx).
- [ ] Frontend: кнопка «Войти через Nostr» на LoginPage/RegisterPage, `window.nostr.getPublicKey()` + `signEvent()` (NIP-07). Расширения нет → внятное сообщение со ссылкой, не пустая ошибка.

**Тесты:** успешный вход · чужая подпись → 401 · подмена challenge → 401 · **повторное использование challenge → 401** · просроченный challenge → 401 · `created_at` вне окна → 401 · неизвестный pubkey → 404 · signup создаёт self-custody-аккаунт без пароля и без email.

**Acceptance:** владелец ключа входит без пароля и без email; перехваченный подписанный запрос повторно не проходит; аккаунт без пароля полностью работоспособен.
