...
- [ ] **Правило «длинных хопов» не реализовано.** `Trip.origin`/`destination` — свободный текст, а не структурированное место; угадывать координаты нечётким поиском по названию хуже, чем не иметь правила. Нужны структурные поля (ISO + geoname id, как у `ReceivingAddress`) — отдельная задача со схемой.
