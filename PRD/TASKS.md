...
### T3.20 — Включить якоря цепи ✅ код (2026-08-01) · ⬜ включение на сервере
...
- [ ] Свой strfry якорь пока не принимает: `refresh_allowed_pubkeys` был запущен в `backend`, у которого нет тома `nostr_whitelist`. Перезапустить в `celery-worker`. На доказательную силу не влияет — она вся в сторонних релеях, — но собственный релей должен хранить и свою копию.
- [ ] Проверить на следующем часовом тике, что beat запускает задачу сам, а не только ручной вызов.
