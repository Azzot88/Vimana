...
### T3.24 — Ридер `.dvlt`: открыть без платформы ✅ MVP (pt.3 — один файл — открыт)
...
**pt.3 — один самодостаточный файл ⬜.** Сейчас собранная страница ссылается на `/assets/reader-*.js`, то есть с диска открывается вместе с этой папкой. `vite-plugin-singlefile` инлайнит всё в один HTML — ставится отдельным шагом, чтобы поведение сборки менялось не вместе с появлением самого ридера.
