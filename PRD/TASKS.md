...
- [ ] **Остаётся владельцу:** задать три значения в `.env` на сервере и `up -d --force-recreate backend celery-worker celery-beat`. До этого якорение — no-op, и ни одна формулировка о независимой проверке в продукте не появится (что правильно: она была бы ложной).
- [ ] После первого тика — проверить, что якоря реально приняты сторонними релеями (`relays` в `deal_chain_anchors`), а не только нашим strfry.
