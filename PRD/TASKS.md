...
- [x] Страница на лендинге (DESIGNGUIDELINES + Bento) — **две версии в репо для сравнения**, обе статические: `frontend/public/dealvault-v3b.html` и `dealvault-v4b.html` (2026-07-26).
- [x] Тон: «Trust is derived from cryptographic evidence» — без обещаний невыключенных фич (публикация якорей — как roadmap, не как факт). В v4b выключенное вынесено в отдельные блоки `.block.road` (пунктир): деньги/эскроу (секция 09), граница проверки (16), портативность и `.dvlt` (17).

**Сделано 2026-07-26 (страницы):**
- Обе версии лежат в `frontend/public/` и попадают в `dist/` штатным копированием Vite (`publicDir` по умолчанию) → отдаются nginx фронта как обычные файлы по `/dealvault-v3b.html` и `/dealvault-v4b.html`. Ни React-роут, ни правка `nginx/default.conf` не требуются: `location /` уже делает `try_files $uri /index.html`, существующий файл выигрывает.
...
- CTA единый на обеих: гость — «Получить инвайт» → `/register`; при живой сессии — «Зайти в аккаунт» → `/dashboard`. Логика в `frontend/public/vimana-cta.js`.
...
**Осталось для закрытия T3.10:** презентация концепции; сравнение v3b/v4b и выбор владельцем; деплой выбранной версии.

**Acceptance:** преза согласована владельцем; лендинг-секция задеплоена; формулировки не заявляют невключённые механизмы как работающие. — *частично: страницы готовы и формулировки сверены с TECHSTATE, преза и выбор версии открыты.*
