...
- [ ] **Никогда:** email, phone, `receiving_*`, содержимое сейфов, список конкретных контрагентов.
...
- [ ] Фронт: `/i/{npub}`, доступна без авторизации. Чип «ключ утрачен» для завершённых.
...
**Кремация — 15 дней:**
- [ ] Тумблер «показывать в архиве», дефолт **да**. В течение 15 дней после `key_lost_at` владелец может выключить; дальше выбор фиксируется.
- [ ] **Что именно исчезает:** страница личности, `display_name`, участие в агрегатах архива. **Что остаётся:** цепь, подписи, события сделок — по ENVIRONMENT §8.2 это критические данные, и они наполовину принадлежат второй стороне. Формулировка в UI обязана это различать, иначе «кремация» обещает больше, чем возможно.
...
- [ ] Килограммы и километры — **после** структурных origin/destination (см. «Открытые зависимости»). Без них расстояние считать не по чему.
...
- **Структурные origin/destination** (ISO + geoname id, как у `ReceivingAddress`). Блокируют уже **две** фичи: правило «длинных хопов» в фильтре публикации (T3.12) и километры в архиве (T3.19). Свободный текст «Tbilisi» не даёт считать расстояние, а нечёткий поиск по названию хуже отсутствия правила. Отдельная задача со схемой и миграцией.
