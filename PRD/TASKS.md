...
### T_PERF.1 — Форма доступа к данным: индексы, N+1, блокирующий I/O 🟨 pt.1 готов
...
**pt.3 — память, счётчики, лимиты ⬜**

- [ ] `core/airports.py` — `_CITY_INDEX` строится из 8,4 МБ GeoNames при импорте модуля и используется **только** внутри `_load()`; после загрузки это мёртвая память на всё время жизни процесса. Инстанс 2 ГБ, OOM уже случался.
- [ ] Там же: `search()` лоуеркейсит 7 000 записей на каждый вызов автокомплита — предвычислить при загрузке; `nearest()` сортирует весь список вместо `heapq.nsmallest`; `list_countries()`/`list_cities()` пересчитывают статику без `lru_cache`.
- [ ] `core/metrics.bump_publish_metric` — read-modify-write в Python при докстринге, обещающем атомарный `UPDATE ... + 1`; при конкуренции инкременты теряются. Заодно поправить имя таблицы в докстринге (`metrics_counters` → `publish_metrics`).
- [ ] `core/rate_limit.py` — slowapi без `storage_uri`: счётчики в памяти процесса, обнуляются каждым деплоем. Redis в стеке уже есть (и теперь под паролем — брать URL из настроек).
