...
- [~] **Под step-up:** `declare-lost` ✅, отвязка passkey ✅. Смена email и добавление нового способа входа — **открыто**: соответствующих эндпоинтов ещё нет, подключить при их появлении. `establish` — **нет**: он уже требует доказательства владения новым ключом, и второе подтверждение ничего не добавляет.
...
- [ ] `ProfilePage.tsx` — отдельная секция «Безопасность» единым списком способов входа. Сейчас они разведены по двум секциям (`KeypairSection` — личность и npub, `PasskeySection` — устройства); визуальное разделение личности и способов входа достигнуто, сведение в один блок — открыто.
