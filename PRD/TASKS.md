...
**Осталось:** поля ввода для `.dvlt` и строк `nsec`/`ncryptsec` — сейчас принести свой ключ можно только расширением NIP-07. Публичный показ смены ключа контрагенту — в `T3.18` (страница личности).
