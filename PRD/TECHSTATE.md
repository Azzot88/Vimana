...
| Правила коридора: четыре корпуса RU/US, вопросы и ответы | 3.11 | ✅ MVP (T3.11.04, T3.11.05, 2026-08-30) |
| Правила коридора: публичный каталог с поиском и фасетами | 3.11 | ✅ MVP (T3.11.14, 2026-08-30) |
| `/rules/*` серверным рендерингом из БД | — | ✅ MVP (T_OPS.2, 2026-09-01) |
| Правила коридора: чеклист, пакет, наблюдение, MCP | 3.11 | ⬜ не начато (T3.11.06 … T3.11.13) |
| Ядро рейса: сегменты, две вместимости, кто летит, приём/выдача | 3.11 | ✅ MVP (T3.11.15, 2026-09-06) |
| Форма публикации рейса: переработка под разбор переписок | 3.11 | 🟨 маршрут-цепочка, две вместимости, два конца передачи, экспресс-путь, запреты пресетом готовы; животные, модель расчёта, Nostr-цепочка — нет (T3.11.07, 2026-09-06) |
| Жизненный цикл рейса: поднятие, отмена, истечение | 3.11 | ⬜ не начато (T3.11.16, 2026-09-06) |
| Ноги груза до вылета и после прилёта | 3.11 | ⬜ не начато, ждёт подъёма к USERJOURNEY §5 (T3.11.17, 2026-09-06) |
| Выкуп: лимит суммы и кто платит за товар | 3.11 | ⬜ не начато, ждёт решения владельца по Фазе 5 (T3.11.18, 2026-09-06) |
| Запрос отправителя и подписка на коридор | 3.11 | ⬜ не начато (T3.11.19, 2026-09-06) |
| Справочники почтовых служб и платёжных систем | 3.11 | ✅ MVP (T3.11.07, 2026-09-06); у почтового нет внешнего источника |
| Три словаря про пересылку почтой + дубль `HANDOVER_METHODS` | 3.11 | ⬜ не начато (T3.11.22, 2026-09-06) |
| Форма рейса как визард из четырёх шагов | 3.11 | 🟨 собран; разделение экрана 3 ждёт A/B и первой тысячи пользователей (T3.11.20, 2026-09-06) |
| Шаблоны рейса: «как в прошлый раз» | 3.11 | ✅ MVP (T3.11.21, 2026-09-06); сохранённые шаблоны и календарь → T6.1 (премиум) |
| Ноги груза: сдача в почту и пред-нога выкупа | 3.11 | ⬜ не начато, блокер снят Этапом 4a USERJOURNEY (T3.11.17, 2026-09-06) |
| Язык доверия: словарь рынка в копирайте | сквозная | ⬜ не начато (T_UX.24, 2026-09-06) |
...
| D-CARRIER-DONE-AT-TRACKING | **Ответственность перевозчика кончается на присланном коде отправления, а не на подтверждении получателя.** Фото делается **до заклеивания** посылки, затем код; с кодом сделку можно отправлять на закрытие. «Сдано в почту» и «доставлено» — разные события цепи. | 44,9 % перевозчиков этого рынка отправляют груз дальше почтой внутри страны прибытия, то есть для половины сделок прежний Этап 5 `USERJOURNEY` описывал момент, которого не происходит: получателя перевозчик не видел. Путь без этого этапа оставлял ему выбор между «соврать доставлено» и «висеть в пути». Фото до заклеивания — единственный момент, когда содержимое ещё видно и уже упаковано; код — то, чем дальше отслеживает получатель. Держать сделку открытой за чужой почтовый срок значит наказывать перевозчика за работу почты. Арбитр после этого разбирает **ногу**, а не «слово против слова». | 2026-09-06 |
| D-TEMPLATES-ARE-PREMIUM | **Бесплатно — «как в прошлый раз»; сохранённые шаблоны и календарь перелётов — премиум (`T6.1`).** | Это один продукт для одного человека, того, кто летает регулярно (5,4 % постов рынка пишут об этом прямо), и продавать его половину бесплатно значит обесценить вторую. Подстановка последнего рейса — удобство и стоит одного запроса; несколько именованных наборов и расписание — инструмент, то есть ровно тот тип выгоды, которого `MASTERPLAN §4.1` требует от потока B: невычитаемый из выручки. | 2026-09-06 |
| D-TRIP-IS-A-CHAIN | **Рейс — цепочка сегментов и две независимые вместимости, а не пара городов с одной датой и одним весом.** `TripLeg` списком; `Trip.origin/destination/depart_at` остаются денормализованным первым и последним узлом. Физическое место и таможенный лимит стоимости — два счётчика, каждый со своим состоянием. Кто летит (`self`/`proxy`) объявляется полем, а не текстом. | Разбор 15 128 сообщений рынка (2026-09-06): 36,6 % постов содержат две и более дат, 15,0 % — три и более города, значит одноногая модель неверна не в краевом случае, а в трети объёма. Вместимости кончаются порознь — «лимит на люкс занят» при свободном месте для документов — и одним числом не описываются. «Лично, без посредников» (16,9 %) стоит в тех же постах, что «(летит подруга)», то есть самый ценный сигнал рынка сегодня ничем не подтверждён; peer-verification обязана применяться к тому, кто пересекает границу. | 2026-09-06 |
| D-MARKET-HAS-NO-RISK-WORDS | **Доверие описывается словарём рынка, а не словарём страховщика.** DealVault не продаётся словами «страховка», «ответственность», «риск». | В 15 128 сообщениях «страховка» — 0 упоминаний, «риск» — 0, «компенсация» — 6. Вместо них: «надёжно» 7,8 %, «есть отзывы» 12,0 %, «конфиденциально» 2,3 %. Риск на рынке огромен, словаря для него нет; слово, которого нет в словаре аудитории, не читается, а выглядит прочитанным. | 2026-09-06 |
...
|---|---|
| **Рейс — цепочка, а вместимость — две (T3.11.15, 2026-09-06).** Маршрут приходит на сервер только как `legs`; плоской тройки `origin`/`destination`/`depart_at` на проводе больше нет, чтобы не заводить правило «какой из двух способов главнее». Нормализация живёт в `core/trip_legs`, а не в роутере: те же правила обязаны действовать для CLI-импорта и фикстуры, а правило, которое проверяет только эндпоинт, следующий не-HTTP писатель обходит молча. Коды приводятся к верхнему регистру на записи (`T_PERF.1` — фильтр сравнивает точно), `leg_order` назначается плотно от нуля и **не берётся из запроса**: присланный порядок приходит разреженным и с дубликатами, а уникальный индекс отвергает строку по причине, на которую перевозчик повлиять не может. Цепочка не идёт назад во времени; одинаковая отметка допускается — перевозчик, знающий дату обратного рейса и не знающий часа, обычен, и отказ вернул бы его в свободное поле. Словари проверяются `CHECK`-ограничениями, а не PG-энумами: список коротких значений будет расти, и добавление значения в тип — миграция с блокировкой. | `app/core/trip_legs.py`, `app/models/marketplace.py` (`TripLeg`, `Trip.space_kind`/`size_hint`/`declared_value_status`/`handover_*`), `app/schemas/marketplace.py` (`TripLegIn`, `TripLegOut`, `HandoverSide`), `app/api/trips.py`, `alembic/versions/0059_trip_chain_and_capacities.py`, `tests/test_trip_chain.py`, `tests/conftest.py` (`_ensure_trip_chain`), `frontend/src/api/trips.ts` |
| **Фаззер и вложенные схемы: `filter_too_much` подавлен осознанно (T_TEST.4 / T3.11.15, 2026-09-06).** `POST /api/trips` уронил три фаззерных теста, когда тело рейса обросло **обязательным массивом вложенных объектов** (`legs`) рядом с нullable-`$ref`-объектами передачи: такую форму `hypothesis-jsonschema` целиком закодировать не умеет и переходит на «сгенерировать и отфильтровать», после чего доля годных примеров падает — на пойманном прогоне 50 отброшенных против 4–8 оставленных. Подавлено, а схема не упрощена: выпрямлять верную схему ради генератора — обратный порядок. **Что это стоит:** утверждение тестов (никаких 5xx) не ослабло, а вот покрытие конкретно этого эндпоинта тоньше, чем говорит `max_examples`, — до обработчика доходит меньше разных тел. Записано здесь, чтобы зелёный прогон фаззера не читали как больше, чем он есть. | `backend/tests/test_contract_fuzz.py` (`FUZZ`) |
| **Форма рейса — визард из четырёх шагов в листе (T3.11.20, 2026-09-06).** `WizardSheet` — общий шелл: на телефоне лист на `100dvh` (с `vh` адресная строка считается видимой, лист выше экрана, футер под сгибом), футер в потоке, а не `fixed` (фиксированный уезжает под клавиатуру), тело с прокруткой между шапкой и футером. Шаг живёт **в URL** (`?step=`): системная «назад» и жест назад на телефоне тогда означают «предыдущий шаг», а не «выбросить визард». Ловушка фокуса написана руками — `aria-modal` объявляет остальное инертным для скринридера и ничего не делает для Tab; фокус возвращается открывшему элементу при закрытии. Прогресс нарисован сегментами, а произнесён в `aria-label` `progressbar`: зрячий не читает «шаг 2 из 4» дважды. Подложка не закрывает лист — форму с черновиком не теряют от случайного тапа; выход через крестик, который спрашивает про черновик, и молча закрывается только на пустой форме. Публикация доступна **с первого шага**, а не со второго, как было записано в постановке: `0060` сделала вес необязательным именно затем, чтобы маршрут и дата были публикуемым рейсом, и лишний экран перед доской — та же пошлина. Превью — свёрнутая панель на каждом шаге, а не пятый экран. | `frontend/src/components/WizardSheet.tsx`, `frontend/src/pages/NewTripPage.tsx`, `frontend/src/i18n/locales/*.json` (`trips.wizard.*`) |
| **Два справочника в образе: почтовые службы и платёжные системы (T3.11.07, 2026-09-06).** Одинаковой формы — глобальный набор плюс по-страновой, — поэтому загрузка, кеш и выборка живут одним модулем `core/directories.py`, а данные двумя файлами: предметы меняются по несвязанным причинам, и ревьюер должен видеть один дифф без другого. Читаются с диска один раз и кешируются, как `core/airports`: в рантайме ни один из списков не меняется. **Сеть не трогается ни разу** — справочник, звонящий третьей стороне, поставил бы публикацию рейса в зависимость от чужого аптайма и отдал бы выбор маршрутов нашими перевозчиками в чужие логи. Почтовые подставляются по **стране прибытия** (пересылка дальше случается после посадки), платёжные — по **обоим концам маршрута** (платят двое, и они в разных странах). Местные записи идут выше глобальных: отправляющий по Турции хочет PTT над DHL, а глобальные имена он и так вспомнит. Дедупликация по коду — SEPA числится у четырёх стран и не должна показаться четырежды. | `backend/app/core/directories.py`, `backend/app/api/directories.py`, `backend/app/data/{postal_services,payment_systems}.json`, `backend/tests/test_directories.py` |
| **Подсказки в поле аэропорта: недавние, популярные, рядом (T3.11.07, 2026-09-06).** Пустое поле с курсором просит вспомнить IATA-код по памяти, поэтому при фокусе выпадают три группы, отсортированные по тому, насколько источник знает именно этого человека. Недавние живут в `localStorage` — это удобство одного устройства и ничьё больше дело; чтение и запись в `try/catch`, потому что в приватном окне хранилище бросает, а подсказка не повод падать. Популярные считаются по `trip_legs` **по обоим концам** каждого перелёта: коридор загружен в обе стороны, счёт по вылетам поставил бы обратный рейс в ноль. Кодов запрашивается вчетверо больше нужного и лишние отбрасываются: рейсы публикуются и с кодами, за которыми нет аэропорта (каждая фикстура сьюта так делает), а показать такой код нечем. Геолокация — **только при уже выданном разрешении** (`navigator.permissions.query`): диалог браузера в ответ на фокус учит отвечать «нет», и явная кнопка остаётся способом его выдать. Кнопка геолокации кладёт результат в ту же группу «рядом» и очищает поле — иначе на экране два разных ответа сразу. Группы дедуплицируются по `iata`: один аэропорт под двумя заголовками читается как два разных варианта. | `frontend/src/components/AirportSelect.tsx`, `frontend/src/api/airports.ts` (`popularAirports`), `backend/app/api/airports.py` (`popular_airports`), `backend/tests/test_airports.py` |
| **Форма рейса спрашивает цепочку, две вместимости и два конца передачи (T3.11.07, 2026-09-06).** Ячейка маршрута — список сегментов; добавление подставляет городом отправления город прибытия предыдущего, потому что так устроены три четверти реальных цепочек. Предполётная проверка коридора спрашивается **по каждому сегменту**, а не по паре «первый–последний»: транзитный узел в основном коридоре присутствует всегда (`§3.11.1`), и именно он чаще несёт ограничение; повторы схлопываются по `id`, иначе рейс туда-обратно предупреждает дважды об одном. Вместимость разделена на место и таможенный лимит с состоянием; при «лимит занят» под переключателем стоит строка о том, что рейс всё равно публикуется, — без неё состояние читается как отказ от рейса. Приём и выдача — два блока с кодом города из цепочки в заголовке и точками свободным текстом через запятую; способы взяты из словаря карточек сделки (`cards.opt.*`), чтобы перевозчик не объявил способ, которого не назовёт ни одна карточка. Пустой конец передачи уходит как `null`, а не `{methods: [], points: []}`: молчание и пустой ответ — разные вещи. Черновик в `localStorage` поднят до `v2` — сохранённый по старой форме читался бы как заполненный и не был бы им. | `frontend/src/pages/NewTripPage.tsx`, `frontend/src/lib/format.ts` (`routeChain`), `frontend/src/pages/{TripsPage,CarrierPage,DashboardPage}.tsx`, `frontend/src/test/routeChain.test.ts`, `frontend/src/i18n/locales/*.json` (`trips.*`) |
...
| **`/rules` рендерится по запросу процессом `ssr`, файлы стали полом (T_OPS.2, 2026-09-01).** Пререндер на сборке означал, что правило, опубликованное после деплоя, невидимо краулеру до следующего. Отдельный процесс, а не бэкенд, отдающий HTML: второе дешевле по инфраструктуре и дороже по дублированию — вёрстка появилась бы второй раз, на Python, и разъехалась бы с React молча. Здесь бандл тот же, что исполняет браузер. Три уровня отказа, и ни один не пустая страница: упал `ssr` — nginx отдаёт файл прошлой сборки; упал бэкенд — `ssr` отдаёт оболочку с кодом 200; нет и файла — оболочка SPA. Upstream задан **переменной** плюс `resolver`, иначе nginx резолвит имя при загрузке конфига и не стартует без опоздавшего контейнера. | `frontend/scripts/ssr-server.mjs`, `frontend/nginx.conf`, `frontend/Dockerfile` (стадия `ssr`), `docker-compose.dev.yml` |
| **`--build --force-recreate` не перезапускает пререндер (найдено 2026-09-01).** Слой `RUN npm run build` кешируется по контексту сборки. Исходники не менялись, данные менялись, Docker про базу не знает — и команда, выглядящая как пересборка, ею не является. Это была не мелочь: никакое количество деплоев не обновило бы страницу, а лог при этом выглядит успешным. Устранено самой `T_OPS.2` — данные больше не запекаются в образ; если пререндер всё же нужно прогнать, это `build --no-cache frontend`. | `frontend/Dockerfile`, `PRD/ENVIRONMENT.md` |
| **Строковый параметр пути проверяется по форме до обращения к БД (T_KEYS.1, повторно T3.11.14, 2026-09-01).** Байт `0x00` в адресе доходит до asyncpg, который отвергает его на уровне протокола (`invalid byte sequence for encoding "UTF8"`), и кривой публичный URL превращается в 500. Это **второй** случай одного класса в проекте: сперва `/identities/{npub}`, теперь `/rules/{category}/{direction}/{country}`. Правило: любой строковый параметр пути, попадающий в запрос, проходит проверку формы в начале обработчика и отвечает **404**, а не 422 — публичный адрес держит одну форму ответа, иначе краулер, покалечивший ссылку, узнаёт, какая половина адреса неверна. Оба раза нашёл контрактный фаззер, ни разу не человек. | `api/trust.public_identity`, `api/rules_public._slug_or_404`, `tests/test_rules_public.py`, `tests/test_contract_fuzz.py` |
| **Имя файла для `Content-Disposition` берётся из найденной строки, а не из адреса (T3.11.14, 2026-09-01).** Кавычка в имени закрывает `filename="…"` раньше времени, управляющий символ — это инъекция в заголовок, а не косметика. Собирать заголовок из того, что вернула БД, безопасно **по построению**, а не потому, что какая-то предыдущая функция что-то отсеяла. Побочная польза: файл называется по коридору, а не по регистру, который человек набрал в адресной строке. | `api/rules_public.read_rule_markdown` |
| **Каталог показывает вопросы, а не их количество (T3.11.14, 2026-08-30).** Страницу открывают две аудитории с противоположными нуждами: везущий регулярно ищет свой коридор и дату сверки за три секунды, а первый раз везущий не знает таксономии вообще — «искусство / вывоз / RU» ему не говорит ничего, потому что он думает вопросами, а не категориями. Поэтому строка каталога несёт **настоящие вопросы**, а не счётчик: новичок узнаёт свой, профессионал пролистывает. Одна раскладка на две задачи, без переключателя между двумя режимами одной страницы. Отсюда же поиск по тексту вопросов: `нужно ли разрешение` — это то, что набирают, а `art export` — нет. | `api/rules_public.index` (`QuestionPreview`), `frontend/src/pages/RulesIndexPage.tsx` |
| **Фасеты выводятся из данных и перекрёстно фильтруются (T3.11.14).** Список стран, зашитый в файл, верен в день написания и молча неверен в день следующего коридора: отсутствующий чип выглядит ровно как несуществующий коридор. Каждое измерение считает достижимые значения **с учётом остальных выбранных** — чип, который может только опустошить экран, хуже отсутствующего, потому что нажатие на него читается как «здесь ничего нет». Фильтрация на клиенте: индекс уже загружен одним массивом, круг до сервера на каждое нажатие не отвечает ни на один вопрос, который клиент не решает сам. Переезд — `T_OPS.3`. | `frontend/src/pages/RulesIndexPage.tsx` |
| **Свежесть печатается дважды (T3.11.14).** Дата последней сверки с источником — центральное обещание раздела, и голая дата его не несёт: `12.01.2026` и `30.08.2026` на взгляд одинаковы, а разница между ними решает, стоит ли странице верить. Печатается «сверено вчера» плюс точная дата, и старше полугода помечается. Порог живёт в `lib/format`, а не в двух страницах: это утверждение о корпусе, а не настройка отображения, и разъехавшаяся копия перестала бы предупреждать молча. | `frontend/src/lib/format.ts` (`freshnessOf`, `STALE_AFTER_DAYS`), `pages/{RulesIndexPage,RulesPage}.tsx` |
| **Указатель пререндерится с данными (T3.11.14).** Раньше он рендерился пустым по доводу «список меняется с каждой публикацией, страница всё равно грузит его сама». Довод неверен ровно в одном: указатель — **единственное**, что ссылается на страницы коридоров, поэтому краулер, пришедший по ссылке из подвала, находил страницу без единой исходящей ссылки, а коридоры оставались недостижимы иначе как угадыванием адреса. Короткая жизнь списка — довод чаще пересобирать, а не отдавать пустым. Файл указателя пишется в любом случае: с каталогом, если API ответил, и с заголовком, если нет. | `frontend/src/entry-ssr.renderRulesIndex`, `frontend/scripts/prerender.mjs` |
| **Тире: запрет применяется к латинице, грамматика кириллицы сохраняется (T3.11.14).** Загруженный дизайн-ruleset запрещает `—` полностью как главный признак машинного текста. В русском и украинском тире грамматически обязательно (замена связки, пояснение), и текст без него читается сломанным носителем языка. Поэтому запрет применён к латинским локалям, где ту же работу делают запятая, двоеточие или точка, и к декоративному употреблению везде, включая кириллицу; грамматическое тире в кириллических строках оставлено. Отступление осознанное и записано, а не забыто. | `frontend/src/i18n/locales/*.json` |
...
- `Trip(id, carrier_id→User, origin, destination, depart_at, capacity?, allowed_categories JSON, status)` — `capacity` стал nullable в **T3.11.07** (0060): маршрут — единственное, что обязательно назвать, чтобы рейс нашли; вес в килограммах называют 2,4 % постов рынка. **T3.11.15** добавил вторую вместимость и два конца передачи: `space_kind ∈ {cabin, checked_partial, checked_full, unspecified}`, `size_hint ∈ {small, medium, large}?`, `declared_value_status ∈ {open, exhausted}`, `handover_origin JSON?`, `handover_destination JSON?` (`{methods, points}`). Колонка `allowed_handover_methods` **удалена** (0059): после разделения на два конца она стала вторым способом сказать то же самое. `origin`/`destination`/`depart_at` остались как денормализованные первый и последний узел цепочки.
- `MeetingPlace(id, user_id→User ON DELETE CASCADE, description, is_default, created_at)` — **T3.11.07** (0065). Второй список в кабинете рядом с `ReceivingAddress`: несколько, один по умолчанию, частичный уникальный индекс `WHERE is_default`. Не поле адреса и не его роль: адрес — куда посылка отправляется, со структурой, нужной почте; место встречи — «у метро Фили, у выхода №3», фраза для человека. CRUD лежит в `api/addresses.py` вместе с адресами, чтобы правило «несколько, один дефолтный, удаление дефолтного выдвигает следующий» существовало в одном экземпляре.
- `Trip.excluded JSON?` — **T3.11.07** (0061). Закрытый словарь `tobacco · alcohol · food · luxury · money`, взят из разбора рынка: запреты пишут 5,9 % постов, и состав почти всегда один из этих пяти. `NULL` = «не сказал» (94 % рынка), `[]` было бы «подумал и не исключает ничего». Свободный текст `carriage_rules` остаётся рядом и несёт всё остальное.
- `TripLeg(id, trip_id→Trip ON DELETE CASCADE, leg_order, origin, destination, depart_at, flown_by ∈ {self, proxy})` — **T3.11.15**. Рейс это цепочка: 36,6 % постов рынка содержат две и более дат, 15,0 % — три и более города. `UNIQUE(trip_id, leg_order)`, `CHECK(origin <> destination)`. Имя `leg_order`, а не `order`: голое слово зарезервировано в SQL. На проводе поле называется `order` — клиенту незачем наследовать обход БД.
...
- `Category(id, name_key UNIQUE, is_default, usage_count, sort_order, created_at)` — T1.17; `sort_order` и категории `parcel` / `clothing` добавлены **T3.11.07** (0063). Словарь: `document · parcel · clothing · medicine · electronics · animal · gift · art · other`. Порядок выдачи — `is_default DESC, usage_count DESC, sort_order, name_key`: собственный трафик платформы старше засева, засев решает только холодный старт.
