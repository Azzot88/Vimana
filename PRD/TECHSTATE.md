...
| D-DOCS-EXPOSURE | Prod: `EXPOSE_DOCS=false` → `/docs`, `/redoc`, `/openapi.json` возвращают 404. Dev: остаются открытыми. Nginx security headers + probe-path deny (`.env`, `.git`, `.php`) добавлены в отдельный слой. | Swagger UI = живая карта API + модели данных = разведка для атакующего. Полное отключение проще auth-протокола за ним, dev не страдает. Реализация — T_SEC.1. | 2026-07-17 |
| D-BENTO-BREAKPOINT | Bento 2 колонки на desktop/tablet, 1 колонка на phone **даже landscape**. Определение через `useBentoLayout()` — не через Tailwind width-only breakpoints. Правила: phone = `(max-width: 767px) OR (max-height: 500px AND any-pointer: coarse)`, tablet = 768-1023 + fine pointer, desktop = 1024+. | iPhone Pro Max landscape = 932px попадает в `md:` (768+), Tailwind width-only даёт 2 колонки — противоречит UX-намерению (пользователь развернул для большего размера, не для больше колонок). | 2026-07-17 |
