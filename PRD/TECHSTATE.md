...
| **Смена типа колонки на `users` — это две вещи, а не одна (T3.42).** Миграция чинит прод, `_ensure_*` в `tests/conftest.py` — тестовую БД: `Base.metadata.create_all` создаёт недостающие **таблицы** и никогда не меняет существующие. Пропуск второй половины уронил 340 тестов и 682 ошибки из одной колонки. Новая таблица требует только миграции; изменение старой — обеих. | `tests/conftest.py` (`_ensure_user_roles_column`), `alembic/versions/0056_roles_add_up.py` |
...
| `backend/app/core/permissions.py` | `Permission.RULES_EDIT`, `RULES_PUBLISH`, бандл `_COMPLIANCE_EDITOR_PERMS` | **Объявлены заранее (T3.42), ни один эндпоинт ими пока не защищён.** Роль `compliance_editor` заведена в этой задаче намеренно, чтобы `T3.11.02` не выдавала её старой бесследной механикой. До `T3.11.02` эти права — декларация: аккаунт с ролью проходит `require_perm` нигде, потому что таких мест ещё нет |
