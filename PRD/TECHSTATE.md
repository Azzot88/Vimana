...
| DealVault: презентация + лендинг | 3.6 | 🟨 в работе (T3.10: две статические страницы `frontend/public/dealvault-v{3b,4b}.html` + `vimana-cta.js` готовы. Преза и выбор версии владельцем — открыты) |
...
| Статические лендинги DealVault (T3.10) — `frontend/public/dealvault-v3b.html` + `dealvault-v4b.html` отдаются как обычные файлы: Vite копирует `public/` в `dist/` (`publicDir` по умолчанию), внутренний nginx фронта резолвит их через `try_files $uri` до SPA-fallback'а. Роут в `App.tsx` и правка `nginx/default.conf` не нужны. Страницы вне i18n (только RU) и вне SPA-навигации — осознанно, решение владельца «оставить в html-виде» | `frontend/public/dealvault-v3b.html`, `frontend/public/dealvault-v4b.html` |
| CTA на статических страницах (T3.10) — `#vimana-cta` рендерится как «Получить инвайт» → `/register`; скрипт подменяет на `data-auth-label`/`data-auth-href` («Зайти в аккаунт» → `/dashboard`) при живой сессии. **Auth-куки в проекте нет** — JWT в `localStorage['token']`, уходит `Authorization: Bearer`; скрипт читает куки (`token`/`access_token`/`vimana_token`) первыми на случай будущей cookie-сессии, затем localStorage; JWT `exp` проверяется, протухший токен сессией не считается. **Файл обязан быть внешним**: прод-CSP `script-src 'self'` (T_SEC.2) без `'unsafe-inline'` молча блокирует любой инлайновый `<script>` на этих страницах | `frontend/public/vimana-cta.js` |
