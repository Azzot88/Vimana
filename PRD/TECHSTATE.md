...
| Идентичность и вход (email-подтверждение, npub, Passkeys, step-up, recovery) | 3.7 | 🟨 в работе. **T3.11 ✅ MVP** (email-only регистрация + код подтверждения; телефон убран из auth, `password_hash` nullable, миграция `0028`). T3.12–T3.17 ⬜. Служебный ключ ≠ личность, переход всегда даёт новый ключ — см. `D-KEY-IS-IDENTITY`. Известный дефект, чинится бэкфиллом в T3.12: у арбитра и superuser нет keypair, из-за чего threshold 2-of-3 не собирается (`api/threshold.py:40`). |
...
| **Гоча asyncio-Redis**: клиент `redis.asyncio` привязывается к event loop, в котором создан. Кеш на уровне модуля (`_client`) ломается под pytest-asyncio — он делает новый loop на каждый тест, и со второго идёт `Event loop is closed`. `core/challenge.py` кеширует клиента **по loop'у** с выселением закрытых. `core/token_blacklist.py` имеет ту же проблему, но fail-soft'ит её в warning: под тестами blacklist фактически не работает, и тесты про отзыв токена проходят не по той причине — чинить отдельно, со своей проверкой | `backend/app/core/challenge.py`, `backend/app/core/token_blacklist.py` |
...
| **Гоча celery-beat**: пересборка `celery-beat` перезапускает планировщик, и `cleanup_e2e_users` может отработать сразу — удалив все `@e2e.vimana.local` старше 24 ч вместе с их сделками и рёбрами доверия | `backend/app/tasks/cleanup.py` |
