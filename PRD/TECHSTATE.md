...
| Правила коридора: публичный справочник `/rules/…` | 3.11 | ✅ MVP (T3.11.03, 2026-08-29) |
| Правила коридора: корпуса RU/US, вопросы и ответы, фасеты | 3.11 | ✅ MVP (T3.11.04, T3.11.05, 2026-08-29) |
| Правила коридора: чеклист, пакет, наблюдение, MCP | 3.11 | ⬜ не начато (T3.11.06 … T3.11.13) |
...
|---|---|
| **Публичный справочник: пререндер на сборке + `try_files` (T3.11.03, 2026-08-29).** Пути `/rules/*` не список в исходниках, а то, что опубликовано в БД, поэтому шаг пререндера спрашивает у API и **не валит сборку**, если тот недоступен (ноутбук против compose). Регулярный `location` с фолбэком на оболочку SPA: набор, опубликованный после деплоя, работает для человека и невидим краулеру до ближайшей сборки. Это **вариант A**; правильный — `T_OPS.2`, порог возврата «больше десяти корпусов». | `api/rules_public.py`, `frontend/src/{api/rulesPublic.ts,pages/RulesPage.tsx,entry-ssr.tsx}`, `frontend/scripts/prerender.mjs`, `frontend/nginx.conf`, `tests/test_rules_public.py` |
| **Локаль справочника выбирается посекционно (T3.11.03).** Полуперевод — реальное состояние: переведённые разделы приходят на языке читателя, остальные по-английски, и каждый говорит, какой он; флаг `fallback_locale` поднимается, если хоть что-то на экране не на языке читателя. Страница, притворяющаяся целиком одной или другой, врёт о состоянии корпуса. | `api/rules_public.read_rule`, `pages/RulesPage.tsx` |
...
| **«Незакрытый набор» для загрузчика — это `draft` и `review` (T3.11.04, 2026-08-29).** Совпадение только по `draft` давало сбой, выглядевший как сломанная кнопка: набор, уже отправленный на ревью, `--replace` не трогал, рядом появлялся второй, редактор оставался на старом — публикация заблокирована пробелами, которых в файле уже нет. Файл, загружаемый с флагом, — это корпус; набор, ждущий ревью, он вытесняет. Без флага загрузчик отказывается и **называет статус**, потому что «уже существует» не подсказывает, где смотреть. | `cli/load_rules.py`, `tests/test_load_rules.py` |
| **Вопрос — это индекс в раздел с цитатой, а не новое утверждение (T3.11.05, 2026-08-29).** Корпус, написанный как закон, проверяем и нечитаем; страница ведёт компактным чтением. Право быть коротким ответ получает от `section_anchor`: под ним лежит раздел с органом, документом и дословной цитатой. Поэтому колонка NOT NULL, а **вторая калитка публикации** отвергает набор, где ответ отвечает от несуществующего раздела. Сохранить такой вопрос при этом можно: редактор пишет вопрос, пока раздел ещё в работе, и запрет на сохранение заставил бы работать в чужом порядке — калитка ничего не теряет, обойти её нельзя. Якори сверяются **поверх локалей**: вопрос по-русски вправе указывать на раздел, который пока есть только по-английски. | `models/rules.py` (`RuleQuestion`), `core/rule_status.publication_blockers`, `api/rules_admin.py`, `api/rules_public.py`, `cli/load_rules.py`, `alembic/versions/0058_rule_questions.py`, `frontend/src/{components/RuleQuestionRow.tsx,pages/{RulesPage,AdminRulesPage}.tsx}` |
| **Фасеты справочника выводятся из данных и перекрёстно фильтруются (T3.11.05).** Список стран, зашитый в файл, верен в день написания и молча неверен в день третьего коридора: отсутствующий чип выглядит ровно как несуществующий коридор. Значения берутся `reduce`-ом по выдаче. Каждое измерение при этом считает достижимые значения **с учётом остальных выбранных**: чип, который может только опустошить экран, хуже отсутствующего — нажатие на него читается как «здесь ничего нет». Фильтрация на клиенте: весь индекс уже загружен одним массивом, круг до сервера на каждый чип не отвечает ни на один вопрос, которого клиент не решает сам. Переедет вместе с `T_OPS.2`. | `frontend/src/pages/RulesIndexPage.tsx` |
...
**Фаза 3.11**
- `RuleQuestion(id, rule_set_id→RuleSet, anchor, order, locale, question, answer, section_anchor, created_at)` — T3.11.05. Второе чтение того же корпуса: закон разделами, вопросы — компактно. `section_anchor` **не FK**: он указывает на `rule_sections.anchor`, уникальный только вместе с локалью, а вопрос по-русски обязан уметь указывать на раздел, который пока есть только по-английски. Разрешение проверяется на калитке публикации. Уникальность `(rule_set_id, anchor, locale)` — перевод вопроса это тот же вопрос, поэтому справочник считает якори, а не строки.
