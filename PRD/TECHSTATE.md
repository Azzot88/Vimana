...
| **Гоча celery-beat**: пересборка `celery-beat` перезапускает планировщик, и `cleanup_e2e_users` может отработать сразу — удалив все `@e2e.vimana.local` старше 24 ч вместе с их сделками и рёбрами доверия. Резкое падение числа юзеров после деплоя — это оно, а не потеря данных (2026-07-26: 36 → 14) | `backend/app/tasks/cleanup.py` |
