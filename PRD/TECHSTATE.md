...
| DealVault Protocol: полнота цепи (msgs/files/seal) + контент-валидация + identity-пересечение | 3.6 | 🟨 T3.7 ✅ + T3.8 ✅ MVP (цепь: messages/files/seal + контент-валидация: сигнатуры + Pillow decode на 4 upload-поверхностях). Остался T3.9 (identity-пересечение) |
