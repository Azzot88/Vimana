...
| DealVault Protocol: полнота цепи (msgs/files/seal) + контент-валидация + identity-пересечение | 3.6 | 🟨 T3.7 ✅ MVP (цепь покрывает messages/files, seal при confirm, dispute распечатывает, `verify_content`, anchor.backend). T3.8 (контент-валидация файлов) + T3.9 (identity-пересечение) — не начаты |
| DealVault: презентация + лендинг | 3.6 | ⬜ не начато (T3.10) |
