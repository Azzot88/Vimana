...
- **`ssr` — процесс серверного рендеринга** (`T_OPS.2`, стадия `ssr` в `frontend/Dockerfile`). Отдаёт `/rules` и `/rules/*` из текущего состояния БД, чтобы публикация правила и его видимость поисковику были одним событием. Наружу не публикуется: nginx фронтенда проксирует на него по compose-сети и на 502 отдаёт файл прошлой сборки.
...
- **Карточный процессинг** — конкретный процессор фиксируется в Decision Log при подходе к T5.2.
...
**Антивирус (T3.8 pt.2, включён 2026-08-09):**
- `CLAMAV_HOST` = `172.31.20.59` — **приватный адрес почтового сервера**, а не localhost. Сканер общий с Mailu: база сигнатур весит ~2 ГБ в памяти и уже держится там, вторая копия на `t3.small` не помещается. `CLAMAV_PORT` по умолчанию 3310.
- **Наличие `CLAMAV_HOST` и есть выключатель** — отдельного `CLAMAV_ENABLED` нет намеренно: две настройки, способные разойтись, позволили бы заявлять сканирование, указывая в никуда.
- **Условия, без которых схема недопустима.** clamd не шифрует и не аутентифицирует: по этому соединению едут вложения DealVault открытым текстом, и кто дотянулся до порта — тот пользуется сканером. Поэтому (1) порт проброшен на почтовом хосте **только на приватный адрес** (`172.31.20.59:3310:3310` в `~/dealvault/mailu/docker-compose.override.yml`), (2) в security group почтового инстанса открыт вход 3310 **только от группы инстанса Vimana**, (3) оба инстанса в одном VPC, трафик не покидает приватную сеть AWS.
- ⚠️ **Проброс живёт в `docker-compose.override.yml` у Mailu и обязан пережить его апгрейд.** Апгрейд перегенерирует собственный compose; если override потеряется, сканирование прекратится молча — узнаем по растущей очереди `pending`, а не по ошибке.
- **Принятая связанность:** почтовый сервер стал зависимостью для двух вещей. Ляжет он — не уходит почта и не сканируются файлы. Вторая деградация безопасна (`scan_for_malware` → `pending`, загрузка проходит, файл в очередь, `rescan_pending_attachments` заберёт позже), поэтому размен принят.
...
- ~~`OTP_COUNTRY_ALLOWLIST` / `TURNSTILE_SECRET_KEY` (T3.29)~~ — **вычеркнуты 2026-08-11** вместе с сокращением T3.29 (`D-CHANNELS-EMAIL-AND-CHAT`). Обе защищали от счёта за SMS-pumping; платных каналов не осталось, качать бесплатный канал бессмысленно. Лимиты T3.29 переменных не имеют — пороги в `core/code_limits.py` (`PER_IDENTIFIER`, `IDENTIFIERS_PER_IP`), потому что менять их надо вместе с комментарием, объясняющим цифру, а не втихую через `.env`.

**T_SEC.6 (письмо о входе с нового устройства):**
- `GEOIP_DB_PATH` — путь к файлу MaxMind **GeoLite2 City** внутри контейнера, значение `/geoip/GeoLite2-City.mmdb`. Не задан или файла нет → письмо уходит без строки о месте; это рабочее состояние, а не сломанное.
  - **Где лежит:** `/opt/vimana/data/geoip/GeoLite2-City.mmdb` на сервере, монтируется `./data/geoip:/geoip:ro` **только в `celery-worker`** — `place_for` зовётся из `send_new_device` и больше ниоткуда, а 63 МБ внутри API-контейнера нечем оправдать.
  - **Не в git и не в образе** (в `.gitignore`): лицензия MaxMind не наша, чтобы её перераспространять; файл обновляется по графику MaxMind, а не по нашим релизам.
  - **Обновление** — заменить файл и `up -d --force-recreate celery-worker`. Reader открывается один раз на процесс (`lru_cache`), подмена файла под работающим воркером не подхватится.
  - Внешнего сервиса геолокации нет намеренно: HTTP-запрос к ipinfo означал бы, что адрес каждого входа уезжает третьей стороне.

**T_UX.29 pt.7 (оповещения и пуш):**
- Переменных **нет и не заведено намеренно.** Лента оповещений работает на существующих Postgres и Redis: строки
  в `notifications`, живой канал — pub/sub по тому же `REDIS_URL`, что уже используют лимиты и блок-лист токенов.
- `push_subscriptions` копит endpoint'ы браузеров, но **ничего не отправляет**: для этого нужны VAPID-ключи
  (`VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT`) и согласие пользователя. Обе вещи — отдельное
  решение владельца, и заводить пустые переменные заранее значит заявлять функцию, которой нет (`§9.1`).
- ⚠️ **`location /api/events/` в `nginx/default.conf`** отключает буферизацию и поднимает `proxy_read_timeout`:
  буферизованный поток, который не кончается, не доходит никогда. Правка конфига требует явного
  `restart nginx` — файл смонтирован read-only, пересборка его не подхватит.
...
> [!WARNING]
> **`--build` не перезапускает шаги сборки, если исходники не менялись.** Слой `RUN npm run build` кешируется по контексту, и Docker ничего не знает про базу. Команда, выглядящая как пересборка, ею не будет — а лог при этом успешный (`CACHED [frontend builder] RUN npm run build`).
>
> С `T_OPS.2` данные больше не запекаются в образ, поэтому обычный деплой это не задевает. Но если нужно принудительно прогнать пререндер (например, обновить файлы-фолбэки после публикации):
>
> ```
