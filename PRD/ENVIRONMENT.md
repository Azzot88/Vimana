...
- `GEOIP_DB_PATH` — путь к файлу MaxMind **GeoLite2 City** внутри контейнера, например `/app/data/GeoLite2-City.mmdb`. Не задан или файл отсутствует → письмо просто уходит без строки о месте; всё остальное работает. **Что нужно, чтобы включить:** бесплатный аккаунт на maxmind.com, скачать `GeoLite2-City.mmdb` (~70 МБ), положить рядом с проектом и примонтировать в `docker-compose.yml` как read-only volume к `backend` и `celery-worker` (читает его воркер, не API). База в образ не кладётся — она обновляется по своему графику и лицензии, а не по нашим релизам. Внешнего сервиса геолокации в проекте нет намеренно: HTTP-запрос к ipinfo означал бы, что адрес каждого входа уезжает третьей стороне.
